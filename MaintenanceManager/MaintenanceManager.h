    ...
    namespace Plugin {
        ...
        class MaintenanceManager : public PluginHost::IPlugin, public PluginHost::JSONRPC {
            ...
            public:
                ...
                /* ---- Accessors ---- */
                bool testSetRFC(const char* rfc, const char* value, DATA_TYPE dataType) {
                    return setRFC(rfc, value, dataType);
                }
                bool testReadRFC(const char* rfc) {
                    return readRFC(rfc);
                }
                ...
                void testStartCriticalTasks() {
                    startCriticalTasks();
                }
                pid_t callGetTaskPID(const char* taskname) {
                    return getTaskPID(taskname);
                }
                void callInternetStatusChangeEventHandler(const JsonObject& parameters) {
                    internetStatusChangeEventHandler(parameters);
                }
                int callAbortTask(const char* taskname, int sig_to_send) {
                    return abortTask(taskname, sig_to_send);
                }
                void setUnsolicitedComplete(bool value) {
                    g_unsolicited_complete = value;
                }
